...
class MainActivity : AppCompatActivity() {
    ...
    private val provider = JsonRpcProvider(
        url = BuildConfig.STARKNET_RPC_URL,
        chainId = StarknetChainId.TESTNET,
    )
    private val scope = CoroutineScope(Dispatchers.IO)

    // Starknet ERC-20 ETH contract address (Mainnet/Testnet/Integration)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        accountAddressInput.setText(BuildConfig.STARKNET_ACCOUNT_ADDRESS)
        privateKeyInput.setText(BuildConfig.STARKNET_PRIVATE_KEY)
        recipientAddressInput.setText(BuildConfig.STARKNET_RECIPIENT_ACCOUNT_ADDRESS)
