...
trait ISaltedCounterContract<TContractState> {
    fn t1703793807962_increase_counter(ref self: TContractState, amount: u128);
    fn t1703793807962_decrease_counter(ref self: TContractState, amount: u128);
    fn t1703793807962_get_counter(self: @TContractState) -> u128;
}
...
mod SaltedCounterContract {
    ...
    impl SaltedCounterContract of super::ISaltedCounterContract<ContractState> {
        fn t1703793807962_get_counter(self: @ContractState) -> u128 {
            self.counter.read()
        }

        fn t1703793807962_increase_counter(ref self: ContractState, amount: u128) {
            let current = self.counter.read();
            self.counter.write(current + amount);
            self.emit(CounterIncreased { amount });
        }

        fn t1703793807962_decrease_counter(ref self: ContractState, amount: u128) {
